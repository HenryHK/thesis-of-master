...
Supposing there are two parties, namely buyer and seller, under our scenerio, they fix an elliptic curve over $(\mathbb{G},O,g,+)$ over a field ${\mathbb{Z}_p}$. According to scheme of ECDSA, the secret key is a private integer $d\in{\mathbb{Z}_{\mid G \mid}}$. In our case, both buyer and seller input a security parameter and they both know the public key $pk = d \cdot g$ where $d$ is the secret. Besides, the Seller and Buyer know ${d_B}, {d_S} \in {\mathbb{Z}_{\mid G \mid}}$ where ${d_S} \cdot {d_B} = d (mod {\mid G \mid})$. \cite{goldfeder2015securing} \cite{herzberg1997method} The share protocol uses a commitment scheme in which eller make a commit $Commit({D_S})$ in exchange for buyer's secret key. If buyer's key is correct, seller will open his commitment afterwards. \\
...
Definitely, the number of test samples is not adequate right now due to time issues. We can still conclude that the speed of transcation which indeed the time to derive a commonly agreed UTXOs is satasfying considering that the client is communicating with multiple remote servers across ocean. This proves the feasibility to deploy Red Belly Blockchain into real life applications, not limited to apllication like wallet. 
...
\subsubsection{ZKCP model of RBBC}
